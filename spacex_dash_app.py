...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig_1 = px.pie([failed, success], values= [failed, success], names= ['Failed', 'Success'], title=('Total successul launches at '+str(entered_site)))
        failed = 0
        success = 0
        return fig_1
...
def get_scatter_plot(entered_site):
    if entered_site == 'ALL':
        fig = px.scatter(x = spacex_df['Payload Mass (kg)'], y = spacex_df['class'], title='Payload Mass (kg) vs success including booster version', color='Booster Version Category')
        return fig
    else:
        df = spacex_df[spacex_df['Launch Site'] == str(entered_site)]
        df = df.reset_index()
        fig1 = px.scatter(x = df['Payload Mass (kg)'], y = df['class'], title='Payload Mass (kg) vs success including booster version '+str(entered_site), color='Booster Version Category')
        return fig1
